...
export default function Login() {
  ...
  const submitLogin = async (e: FormEvent) => {
    ...
    setBusy(false);
    if (!res.ok) {
      setError(res.error || 'Sign in failed. Please verify your credentials or contact Hospital Admin.');
      return;
    }
    if (!remember) {
        ...
        <div className="absolute bottom-0 left-0 right-0 p-12">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.2, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <div className="mt-7 flex items-center gap-5 text-white/55 text-[13px] font-medium">
              ...
              <span className="flex items-center gap-1.5"><Lock size={14} className="text-teal-300" /> Admin-managed credentials</span>
            </div>
          </motion.div>
        </div>
      </div>
        ...
        <div className="w-full max-w-[420px]">
          ...
          >
            ...
            <form onSubmit={submitLogin} className="mt-8 space-y-4" noValidate>
              ...
              <div>
                <label className="block text-[13px] font-semibold mb-1.5" htmlFor="password">Password</label>
                <div className="relative">
          ...
          <p className="mt-8 text-center text-xs opacity-50">
            Protected by role-based access · Passwords are created once by Admin
          </p>
        </div>
      </div>
    </div>
